...
`content_keys` 用来标识具体事件或选题，使用小写英文、数字和连字符。金融科技日报中的持续事件必须为每次真实新增进展创建新键，并在键中包含事件日期。英文跟读使用“人物/作品/主题”组合键。
...
主题标签、金融科技主体或来源 URL 在窗口内重复时会产生警告而不是直接失败，因为持续事件可能需要引用同一家公司或官方资料；生成者仍需确认正文只包含新增信息。
